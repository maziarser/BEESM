...
let Control_color = "#795548";
let Math_color = "#FF9800";
let Text_color = "#AD1457";
let InputOutput_color = "#FF5722";
let Audio_color = "#64DD17";
...
let newVar = "#827717";
let Oled_color = "#2979FF";
let Wifi_color = "#0D47A1";
let WetterOnline_color = "#1976D2";
...
reColor(Blockly.Blocks['serial_print'], Control_color);
...
reColor(Blockly.Blocks['lastcheck_de'], Math_color);
reColor(Blockly.Blocks['resettime_de'], Math_color);
...
reColor(Blockly.Blocks['grove_ultrasonic_ranger'], GroveAnalog_color);
reColor(Blockly.Blocks['grove_button'], GroveAnalog_color);
...
reColor(Blockly.Blocks['setup_display_de'], Oled_color);
reColor(Blockly.Blocks['display_println_de'], Oled_color);
reColor(Blockly.Blocks['display_color_de'], Oled_color);
reColor(Blockly.Blocks['display_textsize_de'], Oled_color);
reColor(Blockly.Blocks['display_setcurser_de'], Oled_color);
reColor(Blockly.Blocks['display_clear_de'], Oled_color);
reColor(Blockly.Blocks['display_display_de'], Oled_color);

reColor(Blockly.Blocks['setup_neopixel_de'], neopixel);
reColor(Blockly.Blocks['set_neopixel_brightness_de'], neopixel);
reColor(Blockly.Blocks['set_pixel_color_rgb_de'], neopixel);
reColor(Blockly.Blocks['neopixel_show_de'], neopixel);
reColor(Blockly.Blocks['set_pixel_alphabet'], neopixel);
reColor(Blockly.Blocks['set_pixel_number'], neopixel);
reColor(Blockly.Blocks['neopixel_fade_de'], neopixel);

reColor(Blockly.Blocks['connect_wifi_de'], Wifi_color);

reColor(Blockly.Blocks['getcurrentWeathercondition_de'], WetterOnline_color);
reColor(Blockly.Blocks['getWeatherforecast_de'], WetterOnline_color);
...
reColor(Blockly.Blocks['get_face_emotion_de'], Connection_color);
...
reColor(Blockly.Blocks['raspy_ardu_de'], robot_color);
reColor(Blockly.Blocks['raspy_ardu_read_de'], robot_color);
reColor(Blockly.Blocks['raspy_ardu_write_de'], robot_color);
//--------------------------------
...
function WebSocketBaall() {
    var objects = ["bedroomJack1", "bedroomJack2", "bedroomLight1", "bedroomLight2", "livingJack1", "livingLight1", "livingLight2", "corridorLight", "kitchenLight", "bathroomLight", "bulblamp", "floorlamp", "bathroomLight", "bathroomdoor", "upperLeftDoor", "upperRightDoor", "lowerLeftDoor", "lowerRightDoor", "livingLight3", "tvProgram"];
    ...
    request.onreadystatechange = function() {
        ...
        switch (status) {
            case 200:
                ...
                var output = document.getElementById('output');
                if ( obj.status === 0 ) {
                    alert("Programm erfolgreich in das Board hochgeladen!");
		    ...
		    document.getElementById("run").disabled = false;
                    ...
                    alert("Programm nicht erfolgreich!!");
                    document.getElementById("loader").style.display="none";
		    document.getElementById("run").disabled = false;
                    var str_status = "Status:" + obj.status;
                    var str_output = obj.output;
...
function resetClick() {
    ...
    uploadCode(code, function(status, errorInfo) {
        if (status != 200) {
            alert("Fehler beim Zurücksetzen des Programms: " + errorInfo);
            document.getElementById("loader").style.display="none";
	    document.getElementById("run").disabled = false;
            alert("Fehler beim Zurücksetzen des Programms: " + errorInfo);
        }
...
function uploadClick() {
    if ($('#arduCode').css('visibility') == 'visible' && $('#modifier').css('display') == 'none') {
        ...
        alert("Bereit zum Hochladen in das Board!");
        document.getElementById("loader").style.display="block";
	    document.getElementById("run").disabled = true;
        ...
        uploadCode(code, function(status, errorInfo) {
            if (status == 200) {
                ...
                alert("Fehler beim Hochladen des Programms: " + errorInfo);
                document.getElementById("loader").style.display="none";
		        document.getElementById("run").disabled = false;
            }
        });
